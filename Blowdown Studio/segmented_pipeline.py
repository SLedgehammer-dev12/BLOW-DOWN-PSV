...
def _darcy_friction_factor(reynolds_number: float, relative_roughness: float) -> float:
    ...
    if reynolds_number < 4000.0:
        f_lam = 64.0 / max(reynolds_number, 1.0)
        term = (relative_roughness / 3.7) ** 1.11 + (6.9 / reynolds_number)
        f_turb = 1.0 / (-1.8 * math.log10(max(term, 1e-12))) ** 2
        frac = (reynolds_number - 2000.0) / 2000.0
        return f_lam + (f_turb - f_lam) * frac
    term = (relative_roughness / 3.7) ** 1.11 + (6.9 / reynolds_number)
...
def run_segmented_pipeline_blowdown_simulation(
    ...
    while True:
        ...
        c_max = 0.0
        for idx in range(segment_count):
            if densities[idx] > 1e-9 and cross_section_area_m2 > 0:
                v = outflow_mdot[idx] / (densities[idx] * cross_section_area_m2)
                v_max = max(v_max, v)
            try:
                c = states[idx].speed_sound()
            except Exception:
                c = math.sqrt(max(real_k[idx], 1.01) * R_U * gas_temperatures[idx] / max(mw_values[idx], 1e-9))
            c_max = max(c_max, c)
        ...
        if v_max + c_max > 0.0 and segment_length > 0.0:
            dt_cfl = 0.5 * segment_length / max(v_max + c_max, 1.0)
            dt = min(dt, max(dt_cfl, 0.001))
