...
def find_psv_area_by_flow_rate(inputs):
    ...
    return A_req_m2, is_choked, k, MW, state.hmass(), Z, pr_crit, state.rhomass(), state.smass(), state.speed_sound()
...
def run_blowdown_simulation_v3(inputs, vana_alani_m2, progress_callback=None, abort_flag=None, silent=False):
    ...
    m_fluid = state.rhomass() * V_sys # Direct density approach (more accurate than PV=znRT)
    ...
    if silent: return t
    # Fix: Ensure 'm_fluid' is correctly passed to dataframe and 'state' is available
    df = pd.DataFrame(zaman_serisi + [{'t': t, 'p_sys': p_sys, 'mdot_kg_s': dm_kg_s, 'T_sys': T_sys, 'T_wall': T_wall, 'h_in': h_in, 'rho_g': state.rhomass(), 'm_sys': m_fluid}])
    return df
...
class Application(tk.Tk):
    ...
    def check_for_updates(self, manual=False):
        def _check():
            try:
                ...
                # Smarter check: Only update if latest_version is 'higher' than current
                def v_val(v):
                    import re
    ...
    def create_right_pane(self, parent):
        ...
        # Export Button (Floating or at bottom of right pane)
        btn_frame = ttk.Frame(parent)
        btn_frame.grid(row=1, column=0, sticky="ew", padx=10, pady=(0, 10))
        self.btn_export = ttk.Button(btn_frame, text="Sonuçları Excel/CSV olarak Dışa Aktar", command=self.export_results_to_csv, state=tk.DISABLED)
    ...
    def on_mode_change(self, event=None):
        ...
        # Clear graph panel on mode switch (only if fig already exists)
        if hasattr(self, 'fig'):
            self.fig.clf()
            ax = self.fig.add_subplot(1, 1, 1)
            label = "Blowdown simülasyonu başlatıldığında grafikler burada görünecek." \
                if "Blowdown" in mode else \
                "PSV analizi tamamlandığında grafikler burada görünecek."
        ...
        if "Blowdown" in mode:
            ...
            self.btn_run.config(text="V3 Analizini Başlat (Enerji Balanşı + MDMT)")
            self.btn_abort.grid()
            self.progress.grid()
    ...
    def run_psv_sizing(self):
        try:
            ...
            # find_psv_area_by_flow_rate now returns 10 values in v2.0
            A_req_m2, is_choked, k_val, MW_val, H_mass, Z_val, pr_crit, rho_g, S_mass, c_sound = find_psv_area_by_flow_rate(inputs)
            ...
            self.results_text.insert(tk.END, f"  * Entalpi (H) / Entropi (S)   : {H_mass/1000:.1f} kJ/kg / {S_mass/1000:.3f} kJ/kg.K\n")
            self.results_text.insert(tk.END, f"  * Lokal Ses Hızı (Acoustic c) : {c_sound:.1f} m/s\n\n")
            ...
            if selected_valve:
                api_data_for_chart = load_api526_data() if "API 526" in valve_type else []
                self.plot_psv_graphs(inputs, actual_area_m2, A_req_mm2,
                                     force_N, vana_sayisi, A_req_per_valve_mm2,
                                     api_data_for_chart)
    ...
    def run_calculation_logic(self):
        try:
            ...
            if sim_df is not None:
                ...
                # --- Başlangıç termodinamiği için CoolProp sorgusu ---
                state_init = CP.AbstractState("HEOS", "&".join(self.user_inputs['composition'].keys()))
                state_init.set_mole_fractions(list(self.user_inputs['composition'].values()))
                state_init.update(CP.PT_INPUTS, self.user_inputs['p0_pa'], self.user_inputs['T0_k'])
                rho_init = state_init.rhomass()
                Z_init   = state_init.compressibility_factor()
                k_init   = state_init.cpmass() / state_init.cvmass()
                MW_init  = state_init.molar_mass() * 1000.0
                H_init   = state_init.hmass()
                S_init   = state_init.smass()
                c_init   = state_init.speed_sound()
                
                # --- Kütle Doğrulama Hesaplaması ---
                # 1. EOS Tabanlı (Hassas)
                m_eos = rho_init * self.user_inputs['V_sys']
                # 2. Termodinamik Denklem Tabanlı (PV/znRT)
                R_spec = R_U / MW_init
                m_ideal = (self.user_inputs['p0_pa'] * self.user_inputs['V_sys']) / (Z_init * R_spec * self.user_inputs['T0_k'])
                diff_m_pct = abs(m_eos - m_ideal) / m_eos * 100.0

                # Simülasyon sonu değerleri
                p_son    = sim_df['p_sys'].iloc[-1]
                T_son_g  = sim_df['T_sys'].iloc[-1] - 273.15
                T_son_m  = sim_df['T_wall'].iloc[-1] - 273.15
                m_baslangic = sim_df['m_sys'].iloc[0]
                m_son       = sim_df['m_sys'].iloc[-1]
                delta_m     = m_baslangic - m_son
                mdot_max    = sim_df['mdot_kg_s'].max() * 3600.0
                mdot_son    = sim_df['mdot_kg_s'].iloc[-1] * 3600.0
                vol_max     = (sim_df['mdot_kg_s'] * 3600.0 / sim_df['rho_g']).max()

                # Ortalama Mach (çıkış hattında)
                A_valve_m2 = selected_valve.area_mm2 / 1e6
                try:
                    # Choked flow check
                    pr_crit = (2 / (k_init + 1))**(k_init / (k_init - 1))
                    is_choked = (P_ATM / self.user_inputs['p0_pa']) <= pr_crit
                    
                    if is_choked:
                        mach_throat = 1.0  # Fiziksel limit
                    else:
                        # Subcritical throat mach calculation
                        beta = P_ATM / self.user_inputs['p0_pa']
                        mach_throat = math.sqrt( (2/(k_init-1)) * ( (1/beta)**((k_init-1)/k_init) - 1 ) )
                    
                    # Exit Mach (Tailpipe) Estimation
                    # Gas expands to P_ATM. We assume isenthalpic expansion for velocity estimation.
                    state_out = CP.AbstractState("HEOS", "&".join(self.user_inputs['composition'].keys()))
                    state_out.set_mole_fractions(list(self.user_inputs['composition'].values()))
                    state_out.update(CP.HmassP_INPUTS, H_init, P_ATM)
                    
                    rho_out = state_out.rhomass()
                    c_out   = state_out.speed_sound()
                    W_max_kg_s = mdot_max / 3600.0
                    
                    # Use nominal valve size as a reference 'tailpipe' diameter for AIV check
                    v_out_theoretical = W_max_kg_s / (rho_out * A_valve_m2)
                    mach_avg = min(v_out_theoretical / c_out, 5.0) # Reasonable cap for display
                except:
                    mach_throat = 1.0
                    mach_avg = 0.0
                ...
                self.results_text.delete("1.0", tk.END)
                self.results_text.insert(tk.END, "====== DETAYLI MÜHENDİSLİK ANALİZ RAPORU ======\n", "HEADER")
                self.results_text.insert(tk.END, f"      Blowdown Simülasyonu — {APP_VERSION}\n\n")

                # --- BÖLÜM 1: TERMODİNAMİK ---
                self.results_text.insert(tk.END, "[1] TERMODİNAMİK VE FİZİKSEL ÖZELLİKLER (API 521, Madde 5.15)\n")
                self.results_text.insert(tk.END, f"  * Başlangıç Basıncı (P₀)         : {(self.user_inputs['p0_pa']/1e5):.2f} bara\n")
                self.results_text.insert(tk.END, f"  * Başlangıç Sıcaklığı (T₀)       : {(self.user_inputs['T0_k']-273.15):.2f} °C\n")
                self.results_text.insert(tk.END, f"  * Sıkıştırılabilirlik (Z)         : {Z_init:.4f}\n")
                self.results_text.insert(tk.END, f"  * Özgül Isı Oranı (k=Cp/Cv)      : {k_init:.3f}\n")
                self.results_text.insert(tk.END, f"  * Moleküler Ağırlık (MW)          : {MW_init:.2f} kg/kmol\n")
                self.results_text.insert(tk.END, f"  * Entalpi (H) / Entropi (S)       : {H_init/1000:.1f} kJ/kg / {S_init/1000:.3f} kJ/kg.K\n")
                self.results_text.insert(tk.END, f"  * Başlangıç Ses Hızı (c)          : {c_init:.1f} m/s\n")
                self.results_text.insert(tk.END, f"  * Gaz Yoğunluğu — Başlangıç      : {rho_init:.3f} kg/m³\n\n")

                # --- YENİ BÖLÜM: KÜTLE DOĞRULAMA ---
                self.results_text.insert(tk.END, "[KÜTLE DOĞRULAMA (MASS VERIFICATION)]\n")
                self.results_text.insert(tk.END, f"  * EOS Tabanlı Kütle (rho*V)      : {m_eos:,.2f} kg (Referans)\n")
                self.results_text.insert(tk.END, f"  * Hesaplanan Kütle (PV/znRT)     : {m_ideal:,.2f} kg\n")
                self.results_text.insert(tk.END, f"  * Yüzdesel Sapma (Hassasiyet)     : % {diff_m_pct:.4f}\n")
                self.results_text.insert(tk.END, "    -> Bilgi: Simülasyon, EOS tabanlı kütle ile yürütülmektedir.\n\n")

                # --- BÖLÜM 2: SİSTEM ve ENERJİ DENGESİ ---
                self.results_text.insert(tk.END, "[2] SİSTEM GEOMETRİSİ VE ENERJİ DENGESİ (1. Termodinamik Yasa)\n")
                self.results_text.insert(tk.END, f"  * Sistem Hacmi (V_sys)            : {self.user_inputs['V_sys']:.4f} m³\n")
                if self.user_inputs.get('HT_enabled', False):
                    self.results_text.insert(tk.END, f"  * İç Yüzey Alanı (A_inner)        : {self.user_inputs.get('A_inner',0):.2f} m²\n")
                    self.results_text.insert(tk.END, f"  * Metal Kütlesi (M_steel)          : {self.user_inputs.get('M_steel',0):.1f} kg\n")
                    self.results_text.insert(tk.END, "  * Isıl Analiz                     : AKTİF (Nusselt, Prandtl, Grashof korelasyonları)\n")
                else:
                    self.results_text.insert(tk.END, "  * Isıl Analiz                     : DEVRE DIŞI (Adyabatik)\n")
                self.results_text.insert(tk.END, f"  * Başlangıç Sistem Kütlesi        : {m_baslangic:.2f} kg\n")
                self.results_text.insert(tk.END, f"  * Son Sistem Kütlesi              : {m_son:.2f} kg\n")
                self.results_text.insert(tk.END, f"  * Tahliye Edilen Toplam Kütle (Δm): {delta_m:.2f} kg\n")
                self.results_text.insert(tk.END, f"  * Enerji Dengesi Yöntemi          : API 521 Madde 5.15 — Açık Sistem 1. Yasa\n\n")

                # --- BÖLÜM 3: VANA VE AKIŞ PERFORMANSI ---
                self.results_text.insert(tk.END, "[3] VANA VE AKIŞ PERFORMANSI (API 521 / API 520 Part I)\n")
                self.results_text.insert(tk.END, f"  * Seçilen Vana Tipi               : {v_type_str}\n")
                self.results_text.insert(tk.END, f"  * Seçilen Vana Boyutu             : {v_label}\n")
                self.results_text.insert(tk.END, f"  * Toplam Gerekli Alan (Teorik)    : {A_req_m2*1e6:.2f} mm²\n")
                self.results_text.insert(tk.END, f"  * Seçilen Toplam Alan (Gerçek)    : {total_selected_area*1e6:.2f} mm²\n")
                self.results_text.insert(tk.END, f"  * Kapasite Marjı                  : %{margin:.1f}\n")
                self.results_text.insert(tk.END, f"  * Maks. Kütlesel Tahliye Hızı     : {mdot_max:,.2f} kg/h (Başlangıç)\n")
                self.results_text.insert(tk.END, f"  * Son Kütlesel Tahliye Hızı       : {mdot_son:,.2f} kg/h\n")
                self.results_text.insert(tk.END, f"  * Maks. Hacimsel Tahliye Hızı     : {vol_max:,.2f} m³/h\n")
                self.results_text.insert(tk.END, f"  * Vana Boğazı Hızı (Throat)       : {mach_throat:.2f} Mach\n")
                self.results_text.insert(tk.END, f"  * Tahliye Hattı (Teorik) Mach     : {mach_avg:.2f} Mach\n")
                self.results_text.insert(tk.END, "    -> Limit: API 521 Madde 5.8 gereği max 0.5~0.7 Mach\n")
                if mach_avg > 0.5:
                    self.results_text.insert(tk.END, "    >> UYARI: Akustik titreşim (AIV) riski — boru destekleme incelenmeli!\n", "WARNING")
                else:
                    self.results_text.insert(tk.END, "    >> DURUM: Akustik hız limitleri içerinde. Güvenli.\n")
                self.results_text.insert(tk.END, "\n")

                # --- BÖLÜM 4: SONUÇ ---
                self.results_text.insert(tk.END, "[4] SONUÇ VE GÜVENLİK DEĞERLENDİRMESİ\n")
                self.results_text.insert(tk.END, f"  * Simülasyon Süresi               : {sim_time:.1f} s\n")
                self.results_text.insert(tk.END, f"  * Hedef Blowdown Süresi           : {target_time:.1f} s\n")
                self.results_text.insert(tk.END, f"  * KRİTER                          : {verdict} ({sim_time:.1f}s {operator} {target_time:.1f}s)\n")
                self.results_text.insert(tk.END, f"  * Simülasyon Sonu Basıncı         : {(p_son/1e5):.2f} bara\n")
                self.results_text.insert(tk.END, f"  * Simülasyon Sonu Gaz Sıcaklığı  : {T_son_g:.2f} °C\n")
                self.results_text.insert(tk.END, f"  * Simülasyon Sonu Metal Sıcaklığı: {T_son_m:.2f} °C\n")
                if verdict == "PASS":
                    self.results_text.insert(tk.END, "  >> SİSTEM: Seçilen vana(lar) API 521 blowdown süresini sağlıyor. ✓\n")
                else:
                    self.results_text.insert(tk.END, "  >> HATA: Hedef süre AŞILDI! Vana alanı yetersiz veya vana sayısı artırılmalı.\n", "WARNING")

                # MDMT uyarısı
                if T_son_m < -46.0:
                    self.results_text.insert(tk.END, f"\n  >> MDMT UYARISI: Metal sıcaklığı ({T_son_m:.1f}°C) kritik eşiğin altına düştü!\n"
                                              "     Malzeme seçimi ve gövde kalınlığı API 579 / ASME B31.3 Ek A kapsamında\n"
                                              "     tekrar değerlendirilmeli.\n", "WARNING")
                self.results_text.config(state=tk.DISABLED)
                if verdict == "FAIL":
                    logging.error(f"KRİTİK: Hedef süre aşıldı! ({sim_time:.1f}s > {target_time:.1f}s)")
                
                self.last_sim_df = sim_df
    ...
    def plot_psv_graphs(self, inputs, A_selected_m2, A_req_mm2, force_N_design, vana_sayisi, A_per_valve_mm2, api526_data):
        """4-panel PSV analysis graphs."""
        self.fig.clf()
        axes = self.fig.subplots(2, 2)

        p0 = inputs['p0_pa']
        p_eval = np.linspace(P_ATM * 1.5, p0 * 1.25, 60)
        capacities, forces = [], []
        for px in p_eval:
            try:
                ti = inputs.copy(); ti['p0_pa'] = px
                A_t, _, k_t, MW_t, _, _, _, _ = find_psv_area_by_flow_rate(ti)
                W_t = inputs['W_req_kg_h'] * (A_selected_m2 / A_t)
                capacities.append(W_t)
                F_t = calculate_reaction_force(W_t / 3600.0, inputs['T0_k'], px, A_selected_m2, k_t, MW_t)
                forces.append(F_t / 1000.0)   # kN
            except:
                capacities.append(float('nan'))
                forces.append(float('nan'))

        # 1. Kapasite Eğrisi
        ax1 = axes[0, 0]
        ax1.plot(p_eval / 1e5, capacities, color='crimson', linewidth=2)
        ax1.axvline(x=p0 / 1e5, color='black', linestyle='--', label='Tasarım Basıncı')
        ax1.axhline(y=inputs['W_req_kg_h'], color='blue', linestyle=':', label='Min. İhtiyaç')
        ax1.set_xlabel('Yukarı Akış Basıncı (bara)'); ax1.set_ylabel('Kapasite (kg/h)')
        ax1.set_title('Basınç vs Kapasite (API 520)'); ax1.legend(); ax1.grid(True)

        # 2. Reaksiyon Kuvveti vs Basınç
        ax2 = axes[0, 1]
        ax2.plot(p_eval / 1e5, forces, color='darkorange', linewidth=2)
        ax2.axvline(x=p0 / 1e5, color='black', linestyle='--', label='Tasarım Basıncı')
        ax2.axhline(y=force_N_design / 1000.0, color='red', linestyle=':', label='Tasarım Kuvveti')
        ax2.set_xlabel('Yukarı Akış Basıncı (bara)'); ax2.set_ylabel('Reaksiyon Kuvveti (kN)')
        ax2.set_title('Basınç vs Reaksiyon Kuvveti (API 520 Pt.II)'); ax2.legend(); ax2.grid(True)

        # 3. Orifis Alan Bar Chart
        ax3 = axes[1, 0]
        if api526_data:
            letters = [o.letter for o in api526_data]
            areas   = [o.area_mm2 for o in api526_data]
            colors_b = []
            selected_found = False
            for o in api526_data:
                if not selected_found and o.area_mm2 >= A_per_valve_mm2:
                    colors_b.append('tomato'); selected_found = True
                elif o.area_mm2 >= A_per_valve_mm2:
                    colors_b.append('lightcoral')
                else:
                    colors_b.append('steelblue')
            ax3.bar(letters, areas, color=colors_b, edgecolor='black', linewidth=0.5)
            ax3.axhline(y=A_per_valve_mm2, color='purple', linestyle='--', linewidth=1.5,
                        label=f'Gerekli: {A_per_valve_mm2:.0f} mm²')
            ax3.set_xlabel('API 526 Orifis Harfi'); ax3.set_ylabel('Orifis Alanı (mm²)')
            ax3.set_title('API 526 Standart Orifis Karşılaştırması'); ax3.legend(); ax3.grid(axis='y')
        else:
            ax3.text(0.5, 0.5, 'API 526 verisi yok', ha='center', va='center', transform=ax3.transAxes)
            ax3.axis('off')

        # 4. Backpressure vs Kb Faktörü
        ax4 = axes[1, 1]
        ...
        actual_bp = (inputs.get('p_downstream', P_ATM) / p0) * 100.0
        ax4.plot(bp_pct_range, kb_conv,  color='steelblue',  linewidth=2, label='Konvansiyonel Vana')
        ax4.plot(bp_pct_range, kb_bbell, color='seagreen',   linewidth=2, label='Körüklü Vana')
        ax4.axvline(x=actual_bp, color='red', linestyle='--', label=f'Mevcut BP: {actual_bp:.1f}%')
        ax4.set_xlabel('Karşı Basınç (%)'); ax4.set_ylabel('Kb Faktörü')
        ax4.set_title('Backpressure vs Kb (API 526)'); ax4.legend(); ax4.grid(True)
        ax4.set_ylim(0, 1.1)
    ...
    def export_results_to_csv(self):
        if self.last_sim_df is None:
            messagebox.showwarning("Uyarı", "Dışa aktarılacak simülasyon verisi bulunamadı.")
            return
            
        file_path = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV Dosyası", "*.csv"), ("Dosya", "*.*")])
        if not file_path: return
        
        try:
            # Prepare a more readable CSV with units
            df_export = self.last_sim_df.copy()
            df_export.columns = [
                'Zaman (s)', 'Basınç (Pa)', 'Debi (kg/s)', 
                'Gaz Sıcaklığı (K)', 'Metal Sıcaklığı (K)', 
                'Isı Transfer Katsayısı (W/m2K)', 'Yoğunluk (kg/m3)', 'Sistem Kütlesi (kg)'
            ]
    ...
    def show_methodology(self):
        ...
        content = (
            f"HESAPLAMA METODOLOJİSİ VE STANDARTLAR ({APP_VERSION})\n\n"
            "Bu program, basınçlı sistemlerin güvenli tahliyesini API standartlarına göre "
            "gerçek gaz denklemleriyle analiz eder.\n\n"
            
            "1. API 520 (PSV BOYUTLANDIRMA - GAZ)\n"
            "------------------------------------\n"
            "Kritik akış (Choked Flow) durumu için kullanılan ana denklem (Eq. 8):\n\n"
            "   A = W / [ C * Kd * P1 * Kb * Kc * sqrt(M / (Z * T1)) ]\n\n"
            "Burada:\n"
            "   - C: Gaz sabit katsayısı (k'ya bağlı fonksiyon)\n"
            "   - Kd: Tahliye katsayısı (CD)\n"
            "   - P1: Tahliye basıncı (mutlak)\n"
            "   - M: Moleküler ağırlık\n\n"
            
            "Sub-kritik akış durumu için kullanılan ana denklem (Eq. 10):\n\n"
            "   A = W / [ 735 * F2 * Kd * sqrt( (P1-P2) * rho ) ]\n\n"
            
            "2. API 521 (BLOWDOWN / DEPRESSURISATION)\n"
            "------------------------------------\n"
            "Zamana bağlı basınç ve sıcaklık değişimi, 1. Termodinamik Yasa (Enerji Balansı) "
            "çerçevesinde sayısal integrasyonla (Euler Method) çözülür:\n\n"
            "   dU/dt = Q_in - (m_out * H_out)\n\n"
            "Sistem soğurken metal cidardan (boru duvarı) çekilen ısı hesabı (Nusselt korelasyonu):\n"
            "   Nu = 0.59 * (Gr * Pr)^0.25 (Laminer akış için)\n\n"
            "Bu sayede MDMT (Minimum Design Metal Temperature) analizi yapılır.\n\n"
            
            "3. API 2000 (TANK HAVALANDIRMA)\n"
            "------------------------------------\n"
            "Düşük basınçlı depolama tanklarında ısıl (thermal) inbreathing/outbreathing ve "
            "sıvı transferi etkileri:\n"
            "   V_thermal = Y * V_tank^0.7 * Ri\n"
            "   V_pump = Q_liquid_max / 0.94 (Nm3/h eşdeğeri)\n\n"
            
            "4. TERMODİNAMİK MOTOR (COOLPROP)\n"
            "------------------------------------\n"
            "Program ideal gaz denklemi (PV=nRT) yerine Heos (Helmholtz Energy Equations of State) "
            "tabanlı hassas hesaplama motorunu kullanır. Bu sayede Joule-Thomson etkisi ve "
            "gerçek gaz sıkıştırılabilirliği (Z) her adımda yeniden hesaplanır.\n\n"
            "DİKKAT: Sonuçlar mühendislik değerlendirmesi içindir; resmi tasarım onaylarında "
            "uzman görüşü ve standartların manuel kontrolü şarttır."
        )
