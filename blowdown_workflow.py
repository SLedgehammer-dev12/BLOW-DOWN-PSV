...
def build_blowdown_report(
    *,
    ...
    report_lines = [
        ...
        f"Sonuç                         : {verdict}",
        f"Gerçekleşen süre              : {format_time(sim_time, uf['time'])}",
        f"Hedef süre                    : {format_time(target_time, uf['time'])}",
        f"Süre karşılaştırması          : {format_time(sim_time, uf['time'])} {operator} {format_time(target_time, uf['time'])}",
        "Gaz kompozisyonu              :",
        *composition_lines,
        "",
        ...
        f"Tek vana alanı                : {format_area(selected_valve.area_mm2 / 1e6, uf['area_small'])}",
        f"Toplam gerekli alan           : {format_area(required_area_m2, uf['area_large'])}",
        f"Toplam seçilen alan           : {format_area(total_selected_area_m2, uf['area_large'])}",
        f"Alan marjı                    : {margin:.1f} %",
        "",
        "[3] Basınç Sonuçları",
        f"Başlangıç basıncı             : {format_pressure(initial_pressure_bara * 1e5, uf['pressure'], converter)}",
        f"Hedef basınç                  : {format_pressure(target_pressure_bara * 1e5, uf['pressure'], converter)}",
        f"Son basınç                    : {format_pressure(final_pressure_bara * 1e5, uf['pressure'], converter)}",
        f"Minimum basınç                : {format_pressure(min_pressure_bara * 1e5, uf['pressure'], converter)}",
        "",
        "[4] Sıcaklık Sonuçları",
        f"Başlangıç gaz sıcaklığı       : {format_temperature(initial_temperature_c + 273.15, uf['temperature'], converter)}",
        f"Son gaz sıcaklığı             : {format_temperature(final_temperature_c + 273.15, uf['temperature'], converter)}",
        f"Minimum gaz sıcaklığı         : {format_temperature(min_temperature_c + 273.15, uf['temperature'], converter)}",
        f"Başlangıç duvar sıcaklığı     : {format_temperature(initial_wall_temperature_c + 273.15, uf['temperature'], converter)}",
        f"Son duvar sıcaklığı           : {format_temperature(final_wall_temperature_c + 273.15, uf['temperature'], converter)}",
        f"Minimum duvar sıcaklığı       : {format_temperature(min_wall_temperature_c + 273.15, uf['temperature'], converter)}",
        "",
        "[5] Kütle ve Debi Sonuçları",
        f"Başlangıç sistem kütlesi      : {format_mass(initial_mass_kg, uf['mass'], converter)}",
        f"Son sistem kütlesi            : {format_mass(final_mass_kg, uf['mass'], converter)}",
        f"Toplam tahliye edilen kütle   : {format_mass(discharged_mass_kg, uf['mass'], converter)}",
        f"Başlangıç yoğunluk            : {initial_density:,.3f} kg/m3",
        f"Son yoğunluk                  : {final_density:,.3f} kg/m3",
        f"Maksimum kütlesel debi        : {format_mass_flow(max_mass_flow_kg_h, uf['mass_flow'], converter)}",
        f"Son kütlesel debi             : {format_mass_flow(final_mass_flow_kg_h, uf['mass_flow'], converter)}",
        f"Maksimum hacimsel debi        : {format_vol_flow(max_volumetric_flow_m3_h, uf['vol_flow'])}",
        f"Son hacimsel debi             : {format_vol_flow(final_volumetric_flow_m3_h, uf['vol_flow'])}",
        "",
        "[6] Isıl Sonuçlar",
        f"Minimum iç ısı transfer kats. : {format_htc(min_h_in, uf['htc'])}",
        f"Ortalama iç ısı transfer kats.: {format_htc(avg_h_in, uf['htc'])}",
        f"Maksimum iç ısı transfer kats.: {format_htc(max_h_in, uf['htc'])}",
    ]
